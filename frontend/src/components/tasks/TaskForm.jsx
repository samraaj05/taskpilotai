...
import { Calendar as CalendarIcon, X, Plus, Sparkles, Loader2, Brain, User, AlertTriangle } from 'lucide-react';
...
export default function TaskForm({
  ...
  // Sync state when task prop changes (for editing)
  React.useEffect(() => {
    if (task) {
      setFormData({
        title: task.title || '',
        description: task.description || '',
        project_id: task.project_id || task.id || '',
        status: task.status || 'todo',
        priority: task.priority || 'medium',
        difficulty: task.difficulty || 'medium',
        domain: task.domain || 'other',
