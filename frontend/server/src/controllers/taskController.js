...
const getTasks = async (req, res) => {
    try {
        ...
        if (limit) {
            ...
            const safeLimit = Math.min(requestedLimit, SAFETY_CONFIG.PAGINATION_LIMITS.TASKS);
            query = query.limit(safeLimit);
        } else {
            query = query.limit(SAFETY_CONFIG.PAGINATION_LIMITS.TASKS);
        }
...
const simulateOverdue = async (req, res) => {
    try {
        ...
        if (!SAFETY_CONFIG.BACKGROUND.SIMULATION_ENABLED) {
            return res.status(200).json({ success: true, message: 'Simulation disabled in Safe Mode' });
        }
